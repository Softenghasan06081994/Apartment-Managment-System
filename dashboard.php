...
$total_fund = 0;
//floor count
$result_floor = mysql_query("SELECT count(fid) as total_floor FROM tbl_add_floor WHERE branch_id = '".(int)$_SESSION['objLogin']['branch_id']."'",$link);
if($row_floor_total = mysql_fetch_array($result_floor)){
	$total_floor = $row_floor_total['total_floor'];
}
//unit count
$result_unit = mysql_query("SELECT count(uid) as total_unit FROM tbl_add_unit WHERE branch_id = '".(int)$_SESSION['objLogin']['branch_id']."'",$link);
if($row_unit_total = mysql_fetch_array($result_unit)){
	$total_unit = $row_unit_total['total_unit'];
}
//owner count
$result_owner = mysql_query("SELECT count(ownid) as total_owner FROM tbl_add_owner WHERE branch_id = '".(int)$_SESSION['objLogin']['branch_id']."'",$link);
if($row_owner_total = mysql_fetch_array($result_owner)){
	$total_owner = $row_owner_total['total_owner'];
}
//rented count
$result_rented = mysql_query("SELECT count(rid) as total_rent FROM tbl_add_rent WHERE branch_id = '".(int)$_SESSION['objLogin']['branch_id']."'",$link);
if($row_rented_total = mysql_fetch_array($result_rented)){
	$total_rented = $row_rented_total['total_rent'];
}
//employee count
$result_employee = mysql_query("SELECT count(eid) as total_employee FROM tbl_add_employee WHERE branch_id = '".(int)$_SESSION['objLogin']['branch_id']."'",$link);
if($row_employee_total = mysql_fetch_array($result_employee)){
	$total_employee = $row_employee_total['total_employee'];
}
//fair count
$result_fair = mysql_query("SELECT sum(rent) as total FROM tbl_add_fair WHERE branch_id = '".(int)$_SESSION['objLogin']['branch_id']."'",$link);
...
//maintaince count
$result_mc = mysql_query("SELECT sum(m_amount) as total FROM tbl_add_maintenance_cost WHERE branch_id = '".(int)$_SESSION['objLogin']['branch_id']."'",$link);
...
//fund count
$result_fund = mysql_query("SELECT sum(total_amount) as totals FROM tbl_add_fund WHERE branch_id = '".(int)$_SESSION['objLogin']['branch_id']."'",$link);
...
//comittee count
$result_c = mysql_query("SELECT count(mc_id) as total FROM tbl_add_management_committee WHERE branch_id = '".(int)$_SESSION['objLogin']['branch_id']."'",$link);
if($row_c_total = mysql_fetch_array($result_c)){
	$total_c = $row_c_total['total'];
}
//utility count
$result_ou = mysql_query("SELECT sum(water_bill) as w_bil,sum(electric_bill) as e_bil,sum(gas_bill) as g_bil,sum(security_bill) as s_bil,sum(utility_bill) as u_bil,sum(other_bill) as o_bil FROM tbl_add_fair where type = 'Owner'",$link);
